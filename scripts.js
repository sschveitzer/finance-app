...
// === XP Panel Add-on (drop-in) ===
(function(){
  const $ = s => document.querySelector(s);
  const on = (el, evt, fn) => el && el.addEventListener(evt, fn);

  const panel = $('#cfgPanel');
  const backdrop = $('#xpBackdrop');
  const btnOpen = $('#btnConfig');
  const btnClose = $('#cfgClose');
  ...
  function openPanel(){
    ...
    try {
      if (typeof S !== 'undefined') {
        const tDark = $('#toggleDark'), tHide = $('#toggleHide');
        if (tDark) tDark.checked = !!S.dark;
        if (tHide) tHide.checked = !!S.hide;
      }
    } catch(e){}
  }
  function closePanel(){
  ...
  on($('#toggleDark'), 'change', async (e) => {
    ...
    } catch(err){ console.warn('toggleDark error', err); }
  });

  on($('#toggleHide'), 'change', async (e) => {
    try {
      if (typeof S !== 'undefined') {
        S.hide = !!e.target.checked;
    ...
    } catch(err){ console.warn('toggleHide error', err); }
  });

  // AddCat botão reusa saveCat/loadAll do projeto
  on($('#addCat'), 'click', async () => {
    const input = $('#newCatName');
    const nome = (input?.value || '').trim();
    if (!nome) return;
    try {
      if (typeof saveCat === 'function') await saveCat({ nome });
